...
func TestMutationBatchBinaryRoundTrip(t *testing.T) {
	...
	in.Mutations[0].Props.TextWrap = ui.TextNoWrap
	in.Mutations[0].Props.TextOverflow = ui.TextOverflowEllipsisMiddle
	in.Mutations[0].Props.MaxLines = 3
	in.Mutations[0].Props.Selectable = true
	in.Mutations[0].Props.RichText = string([]byte{1, 0, 0, 0})
	in.Mutations[0].Props.OnLink = 40
	in.Mutations[0].Props.Placeholder = "Search"
	in.Mutations[0].Props.InputMode = ui.InputUncontrolled
	in.Mutations[0].Props.InputKind = ui.InputSearch
	in.Mutations[0].Props.ReturnKey = ui.ReturnKeySearch
	in.Mutations[0].Props.Capitalization = ui.CapitalizeWords
	in.Mutations[0].Props.AutoCorrect = ui.AutoCorrectDisabled
	in.Mutations[0].Props.Secure = true
	in.Mutations[0].Props.Multiline = true
	in.Mutations[0].Props.ReadOnly = true
	in.Mutations[0].Props.Validation = ui.ValidationInvalid
	in.Mutations[0].Props.ErrorText = "Invalid value"
	in.Mutations[0].Props.SelectionStart = 2
	in.Mutations[0].Props.SelectionEnd = 5
	in.Mutations[0].Props.MaxLength = 80
	in.Mutations[0].Props.OnSubmit = 44
	in.Mutations[0].Props.OnSelection = 45
