...
- (instancetype)init {
    if ((self = [super init])) {
        _gnPlaceholder = [UILabel new];
        _gnPlaceholder.textColor = UIColor.placeholderTextColor;
        _gnPlaceholder.translatesAutoresizingMaskIntoConstraints = NO;
        [self addSubview:_gnPlaceholder];
        [NSLayoutConstraint activateConstraints:@[
            [_gnPlaceholder.leadingAnchor constraintEqualToAnchor:self.leadingAnchor constant:13],
            [_gnPlaceholder.topAnchor constraintEqualToAnchor:self.topAnchor constant:8],
            [_gnPlaceholder.trailingAnchor constraintLessThanOrEqualToAnchor:self.trailingAnchor constant:-8]
        ]];
    }
    return self;
}
...
void GNStyle(uint64_t nodeID, UIView *view, GNNode kind, NSString *text, float width, float height, float padding, float gap, uint8_t alignment, float fontSize, BOOL bold, uint64_t handler, uint64_t changeHandler, uint64_t toggleHandler, BOOL checked, float progress, NSString *accessibility, NSString *hint, uint8_t role, BOOL focused, BOOL scalesText, NSString *imageSource, uint8_t imageMode, BOOL horizontal, uint8_t textWrap, uint8_t textOverflow, uint32_t maxLines, BOOL selectable, NSData *richText, uint64_t linkHandler, NSString *placeholder, uint8_t inputMode, uint8_t inputKind, uint8_t returnKey, uint8_t capitalization, uint8_t autocorrect, BOOL secure, BOOL multiline, BOOL readOnly, uint8_t validation, NSString *errorText, int32_t selectionStart, int32_t selectionEnd, int32_t maxLength, uint64_t submitHandler, uint64_t selectionHandler, NSData *interactions, BOOL animate) {
...
void GNApplyTypedStyle(UIView *view, NSData *payload){
