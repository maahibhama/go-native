...
static CGSize GNMeasureControl(GNNode kind,NSString *text,NSString *imageSource,NSData *typedStyle,CGSize constraint,uint8_t wrap,uint8_t overflow,uint32_t maxLines,BOOL selectable,NSData *richText,NSString *placeholder,uint8_t inputKind,BOOL secure,BOOL multiline,int32_t maxLength){
    UIView *view=GNMake(kind,multiline,selectable||richText.length>0);UIFont *font=GNMeasurementFont(typedStyle,kind==GNButton||kind==GNTextInput?16:17);NSLineBreakMode lineBreak=GNLineBreak(wrap,overflow);
    if([view isKindOfClass:UILabel.class]){UILabel *label=(UILabel *)view;label.text=text;label.font=font;label.numberOfLines=maxLines>0?(NSInteger)maxLines:(wrap==2?1:0);label.lineBreakMode=lineBreak;NSAttributedString *rich=GNRichText(richText,font,lineBreak);if(rich)label.attributedText=rich;}
    ...
    else if([view isKindOfClass:UITextField.class]){UITextField *field=(UITextField *)view;field.font=font;field.placeholder=placeholder;field.keyboardType=GNKeyboardType(inputKind);field.secureTextEntry=secure;NSString *content=text.length?text:(placeholder.length?placeholder:@"M");CGRect bounds=[content boundingRectWithSize:CGSizeMake(CGFLOAT_MAX,CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:font} context:nil];return CGSizeMake(MAX(240,ceil(bounds.size.width)+32),MAX(44,ceil(bounds.size.height)+20));}
    else if([view isKindOfClass:UITextView.class]){UITextView *input=(UITextView *)view;input.font=font;input.text=text.length?text:placeholder;input.textContainer.lineBreakMode=lineBreak;input.textContainer.maximumNumberOfLines=maxLines;NSAttributedString *rich=GNRichText(richText,font,lineBreak);if(rich)input.attributedText=rich;CGSize size=[input sizeThatFits:constraint];return CGSizeMake(MAX(kind==GNTextInput?240:0,ceil(size.width)),MAX(kind==GNTextInput?88:0,ceil(size.height)));}
