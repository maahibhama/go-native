...
typedef NS_ENUM(uint8_t, GNNode) { GNView=1, GNText, GNButton, GNRow, GNColumn };
...
static void GNStyle(UIView *view, GNNode kind, NSString *text, float width, float height, float padding, float gap, uint8_t alignment, float fontSize, BOOL bold, uint64_t handler, NSString *accessibility) {
    ...
    if ([view isKindOfClass:UIButton.class]) { [(UIButton*)view setTitle:text forState:UIControlStateNormal]; GNAction*a=GNActions[@(handler)];if(!a&&handler){a=[GNAction new];a.handler=handler;GNActions[@(handler)]=a;[(UIButton*)view addTarget:a action:@selector(invoke) forControlEvents:UIControlEventTouchUpInside];} }
...
static UIView *GNMake(GNNode kind){UIView*v;if(kind==GNText){UILabel*l=[UILabel new];l.numberOfLines=0;v=l;}else if(kind==GNButton){UIButton*b=[UIButton buttonWithType:UIButtonTypeSystem];v=b;}else if(kind==GNRow||kind==GNColumn){UIStackView*s=[UIStackView new];s.axis=kind==GNRow?UILayoutConstraintAxisHorizontal:UILayoutConstraintAxisVertical;v=s;}else{v=[UIView new];}v.translatesAutoresizingMaskIntoConstraints=NO;return v;}

static void GNApply(NSData *data){GNReader r={(const uint8_t*)data.bytes,(const uint8_t*)data.bytes+data.length};if(u16(&r)!=1)return;uint32_t count=u32(&r);for(uint32_t op=0;op<count;op++){GNMutation mutation=(GNMutation)u8(&r);GNNode kind=(GNNode)u8(&r);uint64_t nodeID=u64(&r),parentID=u64(&r);int32_t index=i32(&r),from=i32(&r);float width=f32(&r),height=f32(&r),padding=f32(&r),gap=f32(&r);uint8_t alignment=u8(&r);BOOL bold=u8(&r);float fontSize=f32(&r);uint64_t handler=u64(&r);NSString*text=str(&r);NSString*accessibility=str(&r);NSNumber*key=@(nodeID);UIView*view=GNViews[key];
    if(mutation==GNCreate){view=GNMake(kind);GNViews[key]=view;GNStyle(view,kind,text,width,height,padding,gap,alignment,fontSize,bold,handler,accessibility);if(!GNRoot.view.subviews.count){[GNRoot.view addSubview:view];[NSLayoutConstraint activateConstraints:@[[view.leadingAnchor constraintEqualToAnchor:GNRoot.view.safeAreaLayoutGuide.leadingAnchor],[view.trailingAnchor constraintEqualToAnchor:GNRoot.view.safeAreaLayoutGuide.trailingAnchor],[view.topAnchor constraintEqualToAnchor:GNRoot.view.safeAreaLayoutGuide.topAnchor]]];}}
    else if(mutation==GNUpdate){GNStyle(view,kind,text,width,height,padding,gap,alignment,fontSize,bold,handler,accessibility);}
    else if(mutation==GNInsert){UIView*parent=GNViews[@(parentID)];if([parent isKindOfClass:UIStackView.class]){UIStackView*s=(UIStackView*)parent;[s insertArrangedSubview:view atIndex:MIN((NSUInteger)MAX(index,0),s.arrangedSubviews.count)];}else{[parent insertSubview:view atIndex:MIN((NSUInteger)MAX(index,0),parent.subviews.count)];}}
    ...
    else if(mutation==GNDelete){[view removeFromSuperview];[GNViews removeObjectForKey:key];if(handler)[GNActions removeObjectForKey:@(handler)];}
    (void)from;
}}
