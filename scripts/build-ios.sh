...
xcrun --sdk iphonesimulator clang -target arm64-apple-ios15.0-simulator -isysroot "$SDK" -fobjc-arc -framework UIKit -framework Foundation -framework CoreGraphics -I"$BUILD" -I"$ROOT/platform/ios" "$ROOT/platform/ios/main.m" "$ROOT/platform/ios/GNProtocolReader.m" "$ROOT/platform/ios/GoNativeRenderer.m" "$BUILD/counter.a" -o "$APP/GoNativeCounter"
